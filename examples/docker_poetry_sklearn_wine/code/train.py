...
if __name__ == "__main__":
    ...
    def register_model(model_name):
        ...
        # client = MlflowClient()
        # client = MlflowClient(tracking_uri="", registry_uri=registry_uri)
        
        # model = client.create_registered_model(model_name)
        # client.create_model_version(name=model_name)
        # mlflow.register_model(model_uri, model_name, model_description)
        model_uri = f"runs:/{model_name}"
        model_description = f"{model_name}"
    ...
    with mlflow.start_run():
        ...
        if tracking_url_type_store != "file":
            ...
            mlflow.sklearn.log_model(
                lr, "model", registered_model_name=model_name, signature=signature
            )
        else:
